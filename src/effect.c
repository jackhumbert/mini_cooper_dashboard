...
static bool should_update_effect = false;

static void effect_callback(lv_timer_t * timer) {
    if (should_update_effect && true) {
        lv_obj_t * canvas = timer->user_data;
        should_update_effect = false;
        lv_snapshot_take_to_buf(canvas, LV_IMG_CF_TRUE_COLOR, &img, &cbuf_tmp, cbuf_temp_size);
        ...
        // lv_canvas_blur_hor(effect1, NULL, 2);
        // lv_canvas_blur_ver(effect1, NULL, 2);
        lv_snapshot_take_to_buf(effect1, LV_IMG_CF_TRUE_COLOR, &img, &cbuf_tmp, cbuf_temp_size);
...
void setup_effect(lv_obj_t * canvas) {
    ...
    cbuf_tmp = (lv_color_t *)ps_malloc(LV_CANVAS_BUF_SIZE_TRUE_COLOR(CANVAS_WIDTH, CANVAS_HEIGHT));
    cbuf_temp_size = LV_CANVAS_BUF_SIZE_TRUE_COLOR(CANVAS_WIDTH, CANVAS_HEIGHT);
    ...
    lv_color_t * ebuf1 = (lv_color_t *)ps_malloc(LV_CANVAS_BUF_SIZE_TRUE_COLOR(CANVAS_WIDTH, CANVAS_HEIGHT));
    ...
    lv_color_t * ebuf2 = (lv_color_t *)ps_malloc(LV_CANVAS_BUF_SIZE_TRUE_COLOR(CANVAS_WIDTH, CANVAS_HEIGHT));
    ...
    lv_timer_t * timer = lv_timer_create(effect_callback, 50, canvas);
}
