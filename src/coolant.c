...
lv_obj_t * coolant_create(lv_obj_t * parent) {
    ...
    lv_obj_set_size(coolant_cont, 130, 100);
...

{
    ...
    lv_obj_align(engine_temp_label, LV_ALIGN_TOP_LEFT, 0, 0);
    ...
    lv_meter_set_scale_range(meter, scale, 250, 30, 270, 270);
    ...
    lv_meter_set_indicator_start_value(meter, rpm_normal_bg, 250);
    lv_meter_set_indicator_end_value(meter, rpm_normal_bg, 30);

    lv_meter_indicator_t * indic1 = lv_meter_add_arc(meter, scale, 10, AMBER_ON, -1);
    lv_meter_set_indicator_start_value(meter, indic1, 140);
    lv_meter_set_indicator_end_value(meter, indic1, 30);
