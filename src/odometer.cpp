...
void Odometer::update(void) {
    ...
    // }
    // km to mi
    // uint64_t decimiles = (get_cache()->odometer + additional_km) / 1.609 * pow(10, decimals);
    // already miles
    if (trip) {
        uint64_t decimiles = get_cache()->trip_odometer * pow(10, decimals - 1);
        if (last_decimiles != decimiles ) {
            ...
            lv_obj_set_width(segments, ODO_SEGMENT_WIDTH * LV_MAX(5, max + 1));
        }
    } else {
        uint64_t decimiles = get_cache()->odometer * pow(10, decimals);
        if (last_decimiles != decimiles ) {
            last_decimiles = decimiles;
            int max = 0;
            for (int i = 0; i < ODO_SEGMENTS; i++) {
            ...
            lv_obj_set_width(segments, ODO_SEGMENT_WIDTH * LV_MAX(7, max + 1));
    }   
    }
}

Odometer::Odometer(lv_obj_t * parent, bool trip, uint8_t decimals) : trip(trip), decimals(decimals) {
    DASH_FONT(RAJDHANI_MEDIUM, 24);
    DASH_FONT(RAJDHANI_SEMIBOLD, 14);

    lv_obj = lv_obj_create(parent);
    lv_obj_set_size(lv_obj, ODO_WIDTH, 50);
    lv_obj_align(lv_obj, LV_ALIGN_TOP_RIGHT, -5, trip ? 210 : 180);
    ...
    lv_obj_set_width(segments, ODO_SEGMENT_WIDTH * (decimals + 1));
