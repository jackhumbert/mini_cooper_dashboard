...
lv_obj_t * tach3_create(lv_obj_t * canvas) {
    ...
    // lv_obj_center(meter);
    lv_obj_set_size(meter, 1850, 105);
    ...
    lv_meter_set_scale_range(meter, scale, 0, 8000, 48, 66 + 180);
    ...
    lv_meter_set_scale_range(meter, label_scale, 0, 8, 48, 66 + 180);

    lv_obj_set_style_text_color(meter, AMBER_ON, 0);
    lv_obj_set_style_text_font(meter, RAJDHANI_REGULAR_24, 0);

    lv_meter_indicator_t * rpm_normal_bg = lv_meter_add_arc(meter, scale, 20, AMBER_OFF, -5);
    lv_meter_set_indicator_start_value(meter, rpm_normal_bg, 0);
    lv_meter_set_indicator_end_value(meter, rpm_normal_bg, REDLINE);

    lv_meter_indicator_t * rpm_red_bg = lv_meter_add_arc(meter, scale, 20, RED_OFF, -5);
    ...
    for (int i = 1; i < 8; i++) {
        lv_meter_indicator_t * line = lv_meter_add_needle_line(meter, scale, 1, lv_color_black(), 0);
        lv_meter_set_indicator_value(meter, line, i * 1000);
    }
